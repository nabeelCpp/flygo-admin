...
const Container = styled(Box)(props => ({
  ...
  boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', // Equivalent to shadow-xl
  display: 'flex',
  justifyContent: 'space-between',
  flexDirection: 'column',
  [props.theme.breakpoints.up('md')]: {
    flexDirection: 'row', // Equivalent to md:flex-row
    gap: '0.5rem' // Equivalent to md:space-x-2
  },
  '& > *:not(:last-child)': {
    marginBottom: '0.5rem' // Equivalent to space-y-2
  }
}))
const Label = styled(Typography)(props => ({
  ...
  color: props.theme.palette.text.secondary // Equivalent to text-gray-500
}))

// const Container = tw.div`md:rounded-xl rounded-xl bg-white p-6 shadow-xl flex justify-between flex-col md:flex-row md:space-x-2 md:space-y-0 space-y-2 border border-gray-300`
const InputCore = tw.input`relative w-full peer flex h-10 focus:outline-none appearance-none border border-gray-300 rounded-full outline-none transition pl-4 pr-6 group-hover:border-green-500 focus:border-green-500 cursor-pointer text-gray-700`
const InputContainer = tw.div`relative w-full md:w-1/3 flex flex-col justify-center items-center pl-2 text-gray-700`
...
md:mt-5 border md:w-full border-gray-500 hover:border-green-500 text-gray-500 hover:text-green-500 focus:border-green-500 focus:text-green-500 transition outline-none`
...
text-gray-900  w-64 max-h-[240px] border z-20 shadow-lg transform transition ease-in-out bg-white rounded-xl overflow-y-auto overflow-x-hidden
...
const OptionContainer = tw(OptionBase)<{
  $active?: boolean
  $selected?: boolean
}>`cursor-pointer transition ${({ $active, $selected }) => ($active || $selected ? 'bg-green-300' : '')}`
...
const GuestOptionComponent = ({ form, name, option }: { form: BookingFormType; name: string; option: GuestOption }) => (
  <OptionBase className='flex items-center justify-between'>
    ...
    <div className='flex items-center justify-center gap-x-2'>
      <GuestButton onClick={form.onPlusClick(option, name)} disabled={form.getIsOptionDisabled(option, 'plus')}>
        <FaPlus className='w-3 h-3' />
      </GuestButton>
      <Text>{option.value}</Text>
      <GuestButton onClick={form.onMinusClick(option, name)} disabled={form.getIsOptionDisabled(option, 'minus')}>
...
const filterAndMapCiies = (query: string) => formattedCities.filter(city => city.value.includes(query.toLowerCase()))
  ...
  return (
    <Container>
      ...
      <InputContainer style={{ width: 'auto' }}>
        <SwapButton title='Swap Locations' aria-label='Swap Locations' onClick={() => form.swapLocations()}>
